...
import com.lijie.service.*;
import com.lijie.utils.FileUtil;
...
public class UserControl {
    ...
    @ResponseBody
    public JSONObject savePersonalDate(@RequestParam("username") String username,@RequestParam("trueName") String trueName,
			@RequestParam("birthday") String birthday,@RequestParam("gender") String gender,
			@RequestParam("email") String email,@RequestParam("personalBrief") String personalBrief,
                                       @AuthenticationPrincipal Principal principal){
    	User user = new User(username, trueName, birthday, gender, email, personalBrief);
        String usernameOld; 
        try {
            usernameOld = principal.getName();
        } catch (NullPointerException e){
            JSONObject jsonObject = new JSONObject();
            logger.info("This user is not login");
            jsonObject.put("status",403);
        ...
        return userService.savePersonalDate(user, usernameOld);
    }
