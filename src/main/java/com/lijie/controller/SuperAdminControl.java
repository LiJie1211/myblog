...
public class SuperAdminControl {
     ...
     * @return
     */
    @PostMapping("/getUserManagement")
    public JSONObject getUserManagement(@AuthenticationPrincipal Principal principal,
                                           @RequestParam("rows") String rows,
                                           @RequestParam("pageNum") String pageNum){
        String username = null;
        JSONObject returnJson = new JSONObject();
        ...
        return userService.getUserManagement(Integer.parseInt(rows), Integer.parseInt(pageNum));
    }
    /**
     * 获得文章点赞信息
     */
