...
public interface UserMapper {
    ...
    @Results({
            ...
            @Result(column = "phone", property = "roles", javaType = List.class, many = @Many(select = "com.lijie.mapper.UserMapper.getRoleNameByPhone")),
    })
    User getUsernameAndRolesByPhone(@Param("phone") String phone);

    @Select("select r.id,r.name from user u LEFT JOIN user_role sru on u.id= sru.User_id LEFT JOIN role r on sru.Role_id=r.id where phone=#{phone}")
